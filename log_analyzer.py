'''
    log analyzer module, has Analyzer class. Instantiated by file path of log file
'''
...
class Log():
    ...
    '''
        Log class is instantiated by single logs generated by Logcat in `threadtime` format
        it cleans the line, and assigns the proper values to the following properties:
        log_time: time
        log_pid: str
        log_tid: str
        log_priority: str
    ...
    def __init__(self, log_line: str, mode: int=None):
        if len(log_line) < 1:
            raise ValueError("Error: Empty Line Detected from Log.")
        # "06-14 22:08:27.424  2987  3046 W System.err:    at java.lang.Thread.run(Thread.java:761)
...
class Analyzer():
    ...
    def return_unique_stats(self):
        '''
            Returns a tuple containing numbers of unique `(warnings, errors, fatals)`
        '''
        return (self.count_unique_warnings, self.count_unique_errors, self.count_unique_fatals)
    ...
    def __calculate_stats(self):
        ...
        for log in self.logs:
            ...
            #         continue
            # if "timed out waiting for ad response" in log.log_message.lower():
            #     time_out_ad += 1
            #     if time_out_ad > 0:
            #         continue

            if "Ads".lower() is log.log_tag.lower() or "MMSDK-PlayList".lower() in log.log_tag.lower():
                if ad_counter > 0:
                    continue
                ad_counter += 1

            if "com.facebook.katana.provider.AttributionIdProvider".lower() in log.log_message.lower():
                if facebook_katana_counter > 0:
                    continue
                facebook_katana_counter += 1
            # print(log.log_message)

            if "attempted to finish an input event" in log.log_message.lower() or "bluetooth" in log.log_message.lower() or "dropping event" in log.log_message.lower() or "cancelling event" in log.log_message.lower() or "discarding hit" in log.log_message.lower():
    ...
    def __str__(self):
        ...
        \nTotal Fatals: {}\nUnique Fatals: {}".format(self.count_warnings,
                                                      self.count_unique_warnings,
                                                      self.count_errors,
                                                      self.count_unique_errors,
                                                      self.count_fatals, self.count_unique_fatals)
